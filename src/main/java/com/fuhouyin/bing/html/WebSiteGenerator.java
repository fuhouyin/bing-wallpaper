package com.fuhouyin.bing.html;
...
import com.fuhouyin.bing.BingFileUtils;
import com.fuhouyin.bing.Images;
...
public class WebSiteGenerator {
    ...
    public String replaceSidebar(String html, Map<String, List<Images>> monthMap, String nowMonth) {
        ...
        for (String month : monthMap.keySet()) {
            String sidabarMenu = HtmlConstant.Sidebar.getSidabarMenuList(month + ".html", month);
            if (month != null && month.equals(nowMonth)) {
                sidabarMenu = sidabarMenu.replace(HtmlConstant.Sidebar.VAR_SIDABAR_COLOR, HtmlConstant.Sidebar.VAR_SIDABAR_NOW_COLOR);
            }
            sidebar.append(sidabarMenu);
        }
        return html.replace(HtmlConstant.Sidebar.VAR_SIDABAR, sidebar.toString());
    }
    ...
    public String replaceHead(String html, Images images, String month) {
        html = html.replace(HtmlConstant.Head.HEAD_IMG_URL, images.getUrl());
        html = html.replace(HtmlConstant.Head.HEAD_IMG_DESC, images.getDesc());
        if (month != null) {
            html = html.replace(HtmlConstant.Head.HEAD_TITLE, "Bing Wallpaper(" + month + ")");
        } else {
            html = html.replace(HtmlConstant.Head.HEAD_TITLE, "Bing Wallpaper");
        }
        return html;
    }
    ...
    public String replaceImgList(String html, List<Images> bingImages) {
        ...
        for (Images bingImage : bingImages) {
            imgList.append(HtmlConstant.ImgCard.getImgCard(bingImage.getUrl(), bingImage.getDate()));
        }
        return html.replace(HtmlConstant.ImgCard.VAR_IMG_CARD_LIST, imgList.toString());
    }
    ...
    public String replaceMonthHistory(String html, Map<String, List<Images>> monthMap, String nowMonth) {
        ...
        for (String month : monthMap.keySet()) {
            String history = HtmlConstant.MonthHistory.getMonthHistory(month + ".html", month);
            if (month != null && month.equals(nowMonth)) {
                history = history.replace(HtmlConstant.MonthHistory.VAR_MONTH_HISTORY_MONTH_COLOR, HtmlConstant.MonthHistory.VAR_MONTH_HISTORY_NOW_MONTH_COLOR);
            }
            monthHistory.append(history + " ");
        }
        return html.replace(HtmlConstant.MonthHistory.VAR_MONTH_HISTORY, monthHistory.toString());
    }
